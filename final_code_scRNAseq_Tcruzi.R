...
png("Figuras/violinplot_numCellExpr_core_subsampledCore.png", width = 16, height = 10, units = 'cm', res = 300)
...
# Set up the PNG device
png("Figuras/violinplot_numCellExpr_core_familias.png", width = 16, height = 10, units = 'cm', res = 300)
...
ggsave("Figuras/lorenz_4groups_coreSub_disruptive_trans_ribosomal.png",
       plot = plot_lorenz_4,
...
png("Figuras/featurePlot_zscore_subsamplecore.png", width = 8, height = 6, units = 'cm', res = 500)
print(p1)
dev.off()

png("Figuras/featurePlot_zscore_disruptive.png", width = 8, height = 6, units = 'cm', res = 500)
print(p2)
dev.off()
...
png("Figuras/featurePlot_zscore_scusampleCore_disruptive.png", width = 24, height = 8, units = 'cm', res = 500)
print(combined_plot)
dev.off()


png("Figuras/featurePlot_zscore_subsampleCore.png", width = 24, height = 16, units = 'cm', res = 500)
print(p1)
dev.off()

png("Figuras/featurePlot_zscore_disruptive.png", width = 24, height = 16, units = 'cm', res = 500)
print(p2)
dev.off()
...
transporters <- read.csv('categories_paper_JBC-2023/transporters_dm28c.csv', sep = ',')
...
# Subset of the genes from the object 
polimerasas <- read.csv('categories_paper_JBC-2023/polymerase_dm28c.csv', sep = ',')
...
# Subset of the genes from the object 
phosphatases <- read.csv('categories_paper_JBC-2023/phosphatases_dm28c.csv', sep = ',')
...
write.csv(gini_summary, "Figuras/gini_cluster0_tripos.csv", row.names = FALSE)
...
# BED files
bed_core <- "revision articulo/localizacion_genomica/Dm28c_core_compartment_positions.bed"
bed_dis  <- "revision articulo/localizacion_genomica/Dm28c_disruptive_compartment_positions.bed"
...
write.csv(res, "revision articulo/localizacion_genomica/genes_region_annotation_tc_gff_simple.csv", row.names = FALSE)
message("Guardado: genes_region_annotation_tc_gff_simple.csv")
head(res)
...
# Lists:
tcs_high_file <- "revision articulo/localizacion_genomica/tcs_high.txt"
tcs_low_file  <- "revision articulo/localizacion_genomica/tcs_low.txt"
...
# --- 4) save output files ----------------------------------
fwrite(neighbors_high, "revision articulo/localizacion_genomica/tcs_high_neighbors.csv")
fwrite(neighbors_low,  "revision articulo/localizacion_genomica/tcs_low_neighbors.csv")
fwrite(immediate_high, "revision articulo/localizacion_genomica/tcs_high_immediate_neighbors.csv")
fwrite(immediate_low,  "revision articulo/localizacion_genomica/tcs_low_immediate_neighbors.csv")
...
# user parameters ---------------------------------------------------
tcs_high_file <- "revision articulo/localizacion_genomica/tcs_high.txt"     # list of gene_id (one column)
...
tcs_low_file <- "revision articulo/localizacion_genomica/tcs_low.txt"     # list of gene_id (one column)
n_focal <- 10                        # how many focal TcS (first n)
family_terms <- c("transialidase","Transialidase","trans-sialidase","Trans-sialidase","mucin","Mucin","masp","MASP","GP63","gp63","gp-63","GP-63",
                  "DGF1","dgf1","DGF-1","dgf-1")
# <-- adjust this vector with the terms that define "multigene families"
...
tcs_low_file  <- "revision articulo/localizacion_genomica/tcs_low_all.txt"
tcs_high_file <- "revision articulo/localizacion_genomica/tcs_high.txt"
...
out_dir <- "revision articulo/localizacion_genomica"
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)
